...
Datum currently uses anycast to direct users to the edge. Anycast is topology-driven: BGP selects the nearest PoP by AS-path, and the user lands wherever the network delivers them. This works well for basic connectivity but has no awareness of what's on the other side of that PoP — no consideration of AI workload placement, data sovereignty rules, real-time congestion, or risk signals.

There is no geo routing today. A user in Frankfurt and a user in São Paulo can land on the same PoP if BGP sees them as equidistant. There is no mechanism to make a deliberate decision based on where the user actually is, what regulations apply, or what compute resources are available nearby.

Traffic Intelligence closes that gap: it adds an intentional, signal-driven decision layer on top of anycast so that routing reflects policy, performance, and AI workload needs — not just BGP topology.

---

## What Traffic Intelligence Is

Traffic Intelligence is the decision layer that sits between inbound traffic and Datum's edge infrastructure. It continuously scores each traffic flow against a set of signals, then selects the optimal path based on a policy hierarchy.

The output is a **routing decision**: which PoP, which cluster, which next-hop — and why.

---

## Phase 1 — Geographic Intelligence

The first phase scopes to geography only. The goal is to make IP-to-geo data reliably available to every Datum system that needs it, and to build the operational foundation (database, update process, distribution) before layering in richer signals.

### Phase 1 Roadmap

Integration is delivered in four stages. The sequence is deliberate — each stage validates the GeoDB and distribution pipeline before the next stage relies on them for higher-stakes decisions.
...
| 3 | **Application Load Balancing** | More complex routing logic. Builds on a distribution pipeline and DB that are already proven in production. |
...
**Update process:** IP-to-geo mappings shift constantly as prefixes are reassigned and carriers restructure. The DB must be refreshed on a defined schedule (typically weekly for commercial feeds) with a tested rollout process — a bad update can flip geo decisions globally.

**GeoDB Source — status: vendor eval required (Ticket #1)**
...
- Lists can reference individual IPs, CIDR blocks, or ASNs
...
Both are distributed through the same replication pipeline to PoP-local stores, so edge components make a single lookup regardless of whether the match comes from geo data or a customer list. However, named list updates require a push path that bypasses the scheduled GeoDB refresh cycle.
...
Phase 1 establishes geo and IP as the foundation. Subsequent phases layer in additional signals to make routing progressively smarter.

### Additional Signals

| Signal | Phase | Description |
|---|---|---|
| **RTT** | 2 | Round-trip time to candidate edge locations — first real latency signal, replaces distance proxy from Phase 1 |
| **Packet Loss** | 2 | Loss rate on candidate paths — distinguishes congestion from distance |
| **Congestion** | 2 | Link utilization at candidate PoPs and upstream — avoids hot paths |
...
| **Model Locality** | 4 | Where the required inference model is currently loaded — avoids cold-start and cross-region transfer |
| **GPU Availability** | 4 | Utilization and capacity at candidate compute nodes — prevents queuing AI workloads on saturated hardware |
