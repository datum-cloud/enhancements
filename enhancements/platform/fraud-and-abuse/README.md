...
For v1, `User` and `AuditLog` are **first-class integrations** in the controller.
The controller has built-in logic to fetch the User resource by name and query the
audit log API for the most recent entry filtered to the target user. This is
intentional -- each source has different query semantics (direct GET vs. filtered
LIST + sort + take latest) that are better expressed in Go than in generic config.

For v1 there is no `dataSources` field on FraudPolicy -- the controller always
queries both sources. A configurable `dataSources` field would be introduced
alongside CEL-based extensibility when additional sources are needed.

At evaluation time, the controller queries each built-in source, merges the
results into a single input map, and passes it to the provider pipeline. If a
...
**v1 data sources (first-class integrations):**
...
**Future extensibility via CEL-based data sources:** When additional data sources are
needed beyond the built-in integrations, a `dataSources` field would be added to the
FraudPolicy spec. CEL expressions would handle field mapping -- the controller fetches
the resource, and CEL maps source-specific fields to well-known input keys without
requiring controller code changes:

```yaml
# Future: dataSources field with CEL-based field mapping
spec:
  dataSources:
    - type: CRD
      resource:
        ...
        kind: BillingProfile
      fieldMappings:
        billingCountry: "object.spec.address.country"
        paymentMethod: "object.spec.paymentType"
```

This keeps v1 simple (2 hardcoded integrations, no dataSources config) while
providing a clear path to user-configurable sources using Kubernetes-idiomatic
CEL expressions.
...
- **Extensible** -- new first-class sources are a controller change; future CEL-based
  sources are purely configuration.
...
**Input fields populated at evaluation (v1):**

| Field | Populated | Data Source |
|---|---|---|
| `ipAddress` | Yes | AuditLog (client IP from audit entry) |
| `emailAddress` | Yes | User (spec.email) |
| `emailDomain` | Yes | User (derived from emailAddress) |
| `firstName` | Yes | User (spec.firstName) |
| `lastName` | Yes | User (spec.lastName) |
| `userAgent` | Yes | AuditLog (client user-agent from audit entry) |
| `acceptLanguage` | Yes | AuditLog (Accept-Language from audit entry) |
| `deviceFingerprint` | No | Future DeviceFingerprint source |
...
- CEL-based data source field mapping for user-configurable sources
